...
export function TerminalImageActionButton({ action, available, refreshing = false, operation, feedback, onClick }: {
  ...
  return <Button size="compact" variant={action === "save" ? "secondary" : "primary"} data-available={available || refreshing} data-state={state} disabled={!available || operation !== null} loading={loading} aria-label={loading ? action === "save" ? "正在保存…" : "正在复制…" : label} onClick={onClick}>
