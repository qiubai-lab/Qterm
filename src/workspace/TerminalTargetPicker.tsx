...
export function TerminalTargetPicker({ profiles, selectedProfileId, status, detail, onSelect, icon = "terminal", localName = "本地终端", localDetail = "系统默认 Shell", ariaContext = "终端连接" }: TerminalTargetPickerProps) {
  ...
  return <div className="terminal-target" ref={rootRef}>
    ...
    {open && <div className="terminal-target-menu" role="menu" aria-label={ariaContext}>
      <button role="menuitemradio" aria-checked={selectedProfileId === null} onClick={() => select(null)}>
