...
describe("application theme contract", () => {
  ...
  it("keeps manager pages and the connection picker on shared theme roles", () => {
    ...
    expect(connectionDialog).toMatch(/\.connection-group-heading:hover[^}]+background:\s*color-mix\(in srgb,\s*var\(--accent-bg\)/);
    ...
    expect(connectionDialog).toMatch(/\.connection-item\.selected:not\(\[data-primary-selected\]\)[^}]+background:var\(--selection-surface\)/);
    expect(connectionDialog).toMatch(/\.connection-group-heading[^}]+color:\s*var\(--accent\)/);
