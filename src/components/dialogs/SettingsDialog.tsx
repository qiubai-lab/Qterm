...
export function SettingsDialog({ onClose, onSecuritySettingsChanged, onTerminalSettingsChanged }: { onClose: () => void; onSecuritySettingsChanged?: (settings: SecuritySettings) => void; onTerminalSettingsChanged?: (settings: TerminalSettings) => void }) {
  ...
  async function persistTerminalSettings(input: Partial<TerminalSettings>) {
    if (!terminal || busy) return;
    setBusy(true); setSaved(false); setError("");
    try {
      const snapshot = await updateTerminalSettings(input);
  ...
  function changeRemoteShellIntegration(enabled: boolean) {
    ...
    void persistTerminalSettings({ remoteShellIntegrationEnabled: false });
  }
  ...
  return <><DialogFrame title="系统设置" subtitle="通用偏好、外观、安全与高级功能" className="settings-dialog" onClose={busy ? () => undefined : close}>
          ...
          {category === "general" ? <div className="settings-general-view">
            ...
            {terminal ? <div className="settings-rows">
              <div className="settings-row">
                <span><strong>OSC 7 终端目录跟踪</strong><small>开启后显示并验证终端目录状态；普通远程登录会识别 Shell，无历史会话在启动时配置 OSC 7 Hook。关闭后不再解析或显示 OSC 7 状态。</small></span>
              ...
              <div className="settings-row">
                <span><strong>无历史 Bash 会话 <span className="settings-experimental-tag">实验功能</span></strong><small>仅对新建和重连的终端生效：使用独立 Bash，不读取或记录命令历史，跳过用户 Shell 配置。OSC 7 可独立开启；不清除已有历史或服务器日志。本地仅支持 Linux / macOS，远程需支持 Bash。</small></span>
                <SettingsSwitch label="无历史 Bash 会话" checked={terminal.historyFreeBashEnabled} disabled={busy} onChange={(enabled) => void persistTerminalSettings({ historyFreeBashEnabled: enabled })}/>
              </div>
    ...
    {confirmRemoteIntegration && <DialogFrame compact className="settings-integration-confirmation" title="开启 OSC 7 终端目录跟踪" subtitle="本地与远程终端" onClose={() => setConfirmRemoteIntegration(false)}>
      <div className="settings-integration-confirmation-body">
        <div className="settings-integration-confirmation-intro"><span aria-hidden="true"><Icon name="terminal" size={17}/></span><p>Qterm 将跟踪终端上报的 OSC 7 目录；普通远程会话先识别 Shell，再注入临时 Hook；无历史 Bash 会话在受控启动时直接配置 Hook。</p></div>
        <ul>
          <li>不会修改远程 <code>.bashrc</code>、Profile 或其他文件</li>
          <li>只缓存目标标识与 Shell 类型，不保存命令输出或凭据</li>
          <li>{terminal?.historyFreeBashEnabled ? "无历史会话初始化失败会阻断连接，不会降级为普通会话" : "瞬态探测失败会自动重试一次，仍失败则继续普通终端连接"}</li>
        </ul>
      </div>
      <footer className="dialog-actions">
        <Button onClick={() => setConfirmRemoteIntegration(false)}>取消</Button>
        <Button variant="primary" data-dialog-autofocus onClick={() => { setConfirmRemoteIntegration(false); void persistTerminalSettings({ remoteShellIntegrationEnabled: true }); }}>确认开启</Button>
      </footer>
    </DialogFrame>}
  </>;
}
