...
export function FileSortHeader({ label, sortKey, sort, className, onChange }: { label: string; sortKey: SortKey; sort: SortState; className?: string; onChange: (key: SortKey) => void }) {
  const direction = sort?.key === sortKey ? sort.direction : null;
  const action = direction === "ascending" ? "已升序，点击降序排列" : direction === "descending" ? "已降序，点击恢复默认排序" : "默认顺序，点击升序排列";
  return <button className={`file-browser-sort-button${className ? ` ${className}` : ""}`} aria-label={`${label}，${action}`} aria-pressed={Boolean(direction)} data-sort-direction={direction ?? undefined} title={`${label}：${action}`} onClick={() => onChange(sortKey)}>
...
function FileRow({ entry, selected, selectionCount, onSelect, onOpen, onContextMenu, onContextMenuKey, position, setSize }: { entry: FileEntry; selected: boolean; selectionCount: number; onSelect: (additive: boolean) => void; onOpen: () => void; onContextMenu: (event: MouseEvent<HTMLButtonElement>) => void; onContextMenuKey: (event: KeyboardEvent<HTMLButtonElement>) => void; position: number | null; setSize?: number }) {
  const [tooltipAnchorRef, descriptionId, hideTooltip, showTooltip, tooltipSurface] = useThemedTooltip<HTMLSpanElement>({ tooltip: entry.name, shouldShow: isElementOverflowing });
  return <><button className="file-row" data-entry-kind={entry.isDirectory ? "directory" : "file"} data-entry-path={entry.path} data-selected={selected || undefined} role="listitem" aria-label={entry.name} aria-selected={selected} aria-describedby={descriptionId} aria-posinset={position === null ? undefined : position + 1} aria-setsize={setSize} style={position === null ? undefined : { transform: `translateY(${FILE_LIST_PADDING + position * FILE_ROW_HEIGHT}px)` }} onFocus={showTooltip} onBlur={hideTooltip} onClick={(event) => {
    hideTooltip();
    ...
    <span className="file-name"><Icon name={entry.isDirectory ? "files" : "file"} size={14}/><span ref={tooltipAnchorRef} onMouseEnter={showTooltip} onMouseLeave={hideTooltip}>{entry.name}</span>{entry.isSymlink && <small>链接</small>}</span>
    <span className="file-size-column">{entry.isDirectory ? "—" : formatSize(entry.size)}</span><span className="file-permission file-permission-column">{formatPermissions(entry.permissionMode)}</span><span className="file-modified-column">{entry.modifiedAt ? new Date(entry.modifiedAt * 1000).toLocaleString() : "—"}</span>
  </button>{tooltipSurface}</>;
}
