...
type RuleType = NetworkRuleInput["type"];

export function NetworkRuleDialog({ profileId, rule, busy, message, onClose, onSave }: { profileId: string; rule: NetworkRule | null; busy: boolean; message: string; onClose: () => void; onSave: (input: NetworkRuleInput) => void }) {
  const [type, setType] = useState<RuleType>(rule?.type ?? "local");
  const [name, setName] = useState(rule?.name ?? "");
  const [bindHost, setBindHost] = useState(rule?.bindHost ?? "127.0.0.1");
  const [bindPort, setBindPort] = useState(String(rule?.bindPort ?? (rule?.type === "socks5" ? 1080 : 8080)));
  ...
  return <DialogFrame title={rule ? "编辑网络规则" : "创建网络规则"} subtitle="配置只保存非敏感端点，启动状态不会持久化" onClose={onClose} dismissible={!busy} className="network-rule-dialog">
    <form className="network-rule-form" onSubmit={submit}>
      <label>类型<select value={type} disabled={busy || Boolean(rule)} onChange={(event) => {
        const nextType = event.target.value as RuleType;
        setType(nextType);
        if (!rule && nextType === "socks5" && bindPort === "8080") setBindPort("1080");
      }}><option value="local">本地端口转发</option><option value="remote">远程端口转发</option><option value="socks5">SOCKS5 动态代理</option></select></label>
      ...
      <footer className="dialog-actions dialog-actions-with-status"><DialogActionStatus message={validation || message}/><div><button type="button" className="secondary-button" disabled={busy} onClick={onClose}>取消</button><button className="primary-button" disabled={busy}>{busy ? "正在保存…" : "保存规则"}</button></div></footer>
    </form>
  </DialogFrame>;
}
