---
id: QB-20260822-network-access-copy-and-browser-proxy
status: archived
archived: 2026-09-02
legacy: true
---
...
让用户从每条网络实例快速取得语义准确、可以直接选择或复制的访问端点，并能在 Windows、macOS 和 Linux 上从运行中的 SOCKS5 实例启动一个明确使用该代理的独立 Chrome 或 Edge 浏览器实例。
...
- Windows、macOS 与 Linux 浏览器代理首期支持 Google Chrome 和 Microsoft Edge；Linux 限定官方原生 `.deb` / `.rpm` 稳定版，弹窗显示浏览器检测、未安装、可启动、启动中、成功和失败状态。
- 浏览器启动使用 Qterm 管理的独立用户数据目录，避免复用或污染用户日常浏览器 Profile。
- 只有 SOCKS5 listener 已实际可用时才启动代理浏览器；浏览器启动后由用户独立管理，停止规则或关闭 Qterm 不强制结束浏览器进程。

## Constraints

- 浏览器代理明确标记为实验性功能，仅支持 Windows、macOS、Linux 桌面版和 Chrome/Edge。
...
- 不支持移动平台或远程启动服务器上的浏览器；Linux 不支持 Snap、Flatpak、Chromium 和非官方浏览器包。
...
6. SOCKS5 浏览器区域显示“实验性”标识和能力边界说明；Windows、macOS、Linux 以外的平台不显示可用启动操作，Firefox 不出现在当期支持列表。
7. Windows、macOS 或 Linux 检测到受支持的 Chrome / Edge 时可从实际可用的 SOCKS5 listener 启动对应独立浏览器实例；未安装、规则未运行、listener 不可达或进程启动失败时不会伪报成功。
...
- 7、8：Rust 单元测试覆盖 Chrome/Edge 平台标识与探测候选、Linux 原生可执行文件筛选、白名单浏览器 DTO、参数构造、独立 Profile 路径、listener 探测和失败映射；三平台实机冒烟验证负责证明真实浏览器可启动。
...
无阻塞问题。首期平台和浏览器范围已确定为 Windows、macOS、Linux 的 Google Chrome 与 Microsoft Edge；Linux 限定官方原生包。

## Recommended Approach

方案一是仅保留 Windows，改动最少但会让同一桌面能力继续割裂。方案二是同时覆盖所有 Linux 包生态，覆盖面最大，但 Snap / Flatpak 沙箱会显著扩大权限、Profile 与测试矩阵。推荐方案三：复用公共 SOCKS 探测、参数和 Profile 逻辑，分别用 Windows 注册表、macOS LaunchServices Bundle ID、Linux PATH 与官方原生安装路径实现受限平台 adapter。该方案不改变网络领域模型或前端契约，同时控制首期兼容范围。
