---
id: QB-20260820-network-rule-type-onboarding
status: archived
archived: 2026-09-02
legacy: true
---
...
- 每个入口同时展示名称、由设备图标、端点角色文字与方向箭头组成的副标题，以及一条面向用户的用途说明。
...
6. 三个入口的副标题不显示 IP、端口或目标地址，但保留设备图标与端点角色文字；SOCKS5 显示“电脑图标 本地 → 服务器图标 服务器网络”，本地转发显示“电脑图标 本地 → 服务器图标 服务器”，远程转发显示相反方向，同时提供完整方向标签供读屏识别。
...
- 6：`NetworkRuleTypeDialog.test.tsx` 覆盖三种方向的可访问文本、可见角色文字、设备图标及地址文字不存在；`appStyles.test.ts` 覆盖副标题的紧凑布局。
...
采用独立的紧凑类型选择弹窗，再挂载类型固定的 `NetworkRuleDialog`。模式数据中保存结构化端点角色和图标，由选择弹窗渲染图标、角色文字与方向箭头并生成无障碍标签；这能保持展示简洁、方向可校验，同时维持选择职责、配置职责和编辑职责分离。
