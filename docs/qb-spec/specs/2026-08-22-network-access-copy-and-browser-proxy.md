...
- 本地转发、远程转发和 SOCKS5 网络实例均显示常驻复制图标按钮。
- 点击复制按钮打开紧凑访问弹窗；地址既可通过按钮写入剪贴板，也可由用户手动选择复制。
- 本地转发展示本地访问地址和服务器目标地址；远程转发展示服务器访问地址和本地目标地址。
...
1. 三种网络实例的复制按钮无需悬停即可发现，具备名称相关的可访问标签、标题、键盘焦点和稳定布局，不影响现有 ON/OFF 开关与右键菜单。
2. 本地转发弹窗显示并可分别复制“本地访问地址”和“服务器目标地址”；远程转发显示并可分别复制“服务器访问地址”和“本地目标地址”。
