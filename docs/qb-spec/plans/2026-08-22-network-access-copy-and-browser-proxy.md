...
为 Local、Remote、SOCKS5 网络实例提供常驻访问复制入口和语义准确的紧凑弹窗，并为 Windows 上运行中的 SOCKS5 规则提供实验性的独立 Chrome/Edge 代理启动能力。
...
- 不支持 Firefox、非 Windows 平台或 Chrome/Edge 以外的浏览器。
...
- `src-tauri/src/infrastructure/browser.rs`（新增）
...
- command 不接受 executable、参数数组或 Profile 路径。浏览器 adapter 根据可信浏览器枚举探测 Windows 安装位置，优先使用注册的 App Paths，并以经过验证的标准安装路径作为回退。
...
- 浏览器安装探测、路径选择、Profile 目录和进程启动是 OS adapter，归 `infrastructure::browser`。
...
- A7 真实启动条件：Rust 测试覆盖 listener 不可达拒绝和 spawn 错误；Windows 实机分别启动 Chrome/Edge 并确认使用当前 SOCKS listener。
...
6. 运行 `git diff --check`。
7. Windows 桌面冒烟检查 Chrome 与 Edge：已安装/未安装检测、运行中 SOCKS 启动、停止规则拒绝、独立 Profile、代理出口、DNS 行为、关闭弹窗焦点恢复和最小窗口布局。外网出口/DNS 验证仅作为手工证据，不进入不稳定的自动化测试。
