...
- `src/lib/tauri/git.ts`：本机及 profile-bound SSH 工作区的快照、初始化、stage/unstage、commit、本地分支操作与 Git-purpose 只读目录浏览窄 IPC 契约；只发送封闭 action union 或 session/profile/path 三元组，不接受 executable、任意 cwd、shell、子命令或参数数组。
...
- `src-tauri/src/infrastructure/ssh/client.rs`、`client/{handler,session,shell_integration,network,transfer,git}.rs`：稳定 `SshSessionManager` façade 与内部 route/host-key handler、purpose-specific session runner、有界 Shell 探测、Git 固定 exec/stdin 协议、Git session 的只读目录 SFTP、Files/Terminal SFTP/transfer 及私有 Terminal 暂存目录实现；generic Files API 不得借用 Git session，第三方 russh/SFTP 类型只在该 infrastructure 子树内，测试位于 `client/tests.rs`。
