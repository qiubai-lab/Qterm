...
- `src/workspace/WorkspaceShell.tsx`：顶部 Workspace 标签、工具轨、Terminal/Files 认证路由、关闭编排和快捷键入口；不实现布局树变换或 SSH 协议。
- `src/workspace/model.ts`、`layout.ts`、`reducer.ts`：可持久化工作区契约、纯布局树变换与低频结构状态；不持有 session、xterm 或凭据。
- `src/workspace/WorkspaceProvider.tsx`：Workspace 持久化与按 `blockId` 隔离的 Terminal/File runtime 编排边界；Files 远程 session 独立于终端，且不保存高频终端字节。
...
- `src-tauri/src/application/`：负责 profile、credential、workspace、host-key 信任和文件内容安全用例；不依赖 Tauri UI 或具体 SSH 库类型。
...
- `src-tauri/src/infrastructure/ssh/`：负责 `russh` 私钥凭证解析、平台 SSH Agent 签名适配、带用途连接生命周期、PTY 和 SFTP；不扫描设备私钥目录，也不泄漏私钥正文或第三方 channel 类型。
