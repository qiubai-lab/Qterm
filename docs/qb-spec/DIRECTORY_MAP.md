...
- `src/git/GitPane.tsx`、`GitRepositoryHistoryPopover.tsx`、`GitRepositoryPickerDialog.tsx`、`gitGraph.ts`：本机或 SSH 工作区单仓库 Git Block 的三段式交互、成功打开报告、最近仓库浮层、目录选择弹窗、手动 fetch 与自动 snapshot 意图区分、本地/远程分支分组、target-aware action、请求竞态/stale 保护和 parent OID 拓扑派生；浮层只展示 Workspace 提供的当前 scope 历史，远程选择器只浏览目录并保留手动路径回退，不拥有持久化/session 生命周期、不复用文件预览/传输、不生成 Git 命令，也不提供 diff、pull、push 或 remote 管理。
...
- `src/lib/tauri/git.ts`：本机及 profile-bound SSH 工作区的快照、初始化、stage/unstage、commit、本地/远程分支引用、主动 fetch、跟踪分支签出与 Git-purpose 只读目录浏览窄 IPC 契约；只发送封闭 action union 或 session/profile/path/ref identity，不接受 executable、任意 cwd、shell、子命令、参数数组、remote URL 或凭据。
...
- `src-tauri/src/domain/git.rs`、`ports/{git_executor,remote_git_executor}.rs`、`application/git_service.rs`：Git 元数据、完整 branch ref/kind、跟踪分支匹配、本机/远程封闭 action 输入边界、同步本机与异步 profile-bound fetch/签出能力端口及用例编排；不依赖 Tauri、process API、shell 或具体 Git 输出格式。
...
- `src-tauri/src/infrastructure/git_cli.rs`：系统 Git 发现、固定结构化 snapshot/fetch/branch 参数、网络与普通操作超时/输出上限及 machine-readable status/ref/log 解析；不经 shell、不接收 WebView 命令片段、参数数组、remote URL/凭据或持久化仓库状态。
...
- `src-tauri/src/infrastructure/ssh/client.rs`、`client/{handler,session,shell_integration,network,transfer,git}.rs`：稳定 `SshSessionManager` façade 与内部 route/host-key handler、purpose-specific session runner、有界 Shell 探测、Git 固定 snapshot/fetch/branch exec/stdin 协议、Git session 的只读目录 SFTP、Files/Terminal SFTP/transfer 及私有 Terminal 暂存目录实现；generic Files API 不得借用 Git session，Git action 不接受任意命令/remote 凭据，第三方 russh/SFTP 类型只在该 infrastructure 子树内，测试位于 `client/tests.rs`。
...
- `src/git/`：负责单仓库 Git Block 展示、Local/Remote target action 选择、手动 fetch 与自动 snapshot 意图、本地/远程分支分组、成功 snapshot 报告、最近仓库浮层、受限目录浏览弹窗、局部交互状态、刷新代次和提交图拓扑；不拥有 Workspace 持久化、session 生命周期或 Git branch 规则，不直接启动 Git、创建 SSH channel、读取文件内容、承接文件管理能力或实现 diff/pull/push/remote 管理。
