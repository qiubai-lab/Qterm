...
4. 应用每次启动时保持凭证库锁定；用户解锁后，Rust 后端按连接引用解析密码或私钥并进入交互终端。初始化时先由应用内确认说明恢复密钥用途，用户明确点击保存后再选择本地位置；恢复文件默认命名为 `qterm-recovery-{Unix 时间戳}.key`。忘记主密码后先加载恢复文件，由 Rust 完成有效性校验；验证成功后才允许输入新主密码，随后返回应用显示新密钥保存确认，用户再次点击后才打开保存窗口并轮换恢复文件。用户只可在凭证管理中明确显示单条密码；手动锁定、Windows 系统锁屏策略或凭证会话到期会清除运行时密钥，系统恢复后不自动解锁。
5. 用户通过同一产品上传或下载远程文件。
