...
- `src/app/styles/themes/{dark,light,cyberpunk}.css` 实现共享 surface/text/border/status、完整 terminal ANSI、editor 与 workbench component semantic token contract。`--signature` 表达主题识别，`--primary-action` 表达主要动作，`--navigation-accent` 表达应用级导航选中强调，`--selection-surface/--selection-marker` 分别表达内容选中底与边界/标记，`--workspace-tab-active-text` 表达顶部当前工作区文字。三套 preset 的活动 Block 标题都必须使用自身强调色的克制渐变，并与活动边框形成同一层级；Workspace 属于应用级导航，当前项使用 accent surface/轮廓/图标与 `text-strong` 文字，不得把 Cyberpunk 的黄色内容 selection marker 混入青色 Workspace 容器。Cyberpunk 的 Settings、连接目标、凭证、文件、页签与 segmented control 内容选中态继续使用黄色轻量 surface 和亮黄标记，未选中功能图标、连接、启用、焦点、浮层边界和滚动反馈使用青色。chrome、utility rail 与 workspace 画布不得用无状态意义的黄色渐变。warning 使用琥珀；danger 只用于失败、删除、不可逆确认及关闭控件的 hover/pressed，不用于普通选中或装饰。跨主题的文字强调/禁用层级、图标、control hover、Block 边界/header、scrollbar，以及 manager sidebar/editor、card、popover/menu 的 surface 与交互状态，必须由 feature CSS 直接消费语义角色，不得由某一主题的 selector 修补；Light 与 Cyberpunk 常规小字角色在基础 surface 上至少保持 4.5:1。遗留 feature 原始暗色值由仅在 Light root 生效的 `lightOverrides.css` 兼容层隔离，新增 feature CSS 受 raw-color budget 测试约束，后续应继续按功能语义收敛而不是扩大兼容层。
