...
- `document.documentElement[data-theme]` 是唯一 theme DOM owner，`src/app/theme/AppThemeProvider.tsx` 是运行期唯一状态 owner，只允许内置 `dark | light | cyberpunk`。Dark 是同步启动 fallback 与缺省值；React 首次渲染前从 Settings snapshot 引导已保存选择，不支持 system/auto/custom theme。Tauri native window 只接收 light/dark，Cyberpunk 由 window adapter 显式映射为 Dark。
- `src/app/styles/themes/{dark,light,cyberpunk}.css` 实现共享 surface/text/border/status、完整 terminal ANSI、editor 与 workbench component semantic token contract。`--signature` 表达主题识别，`--primary-action` 表达主要动作，`--navigation-accent` 表达应用级导航选中强调，`--selection-surface/--selection-marker` 分别表达内容选中底与边界/标记，`--workspace-tab-active-text` 表达顶部当前工作区文字。三套 preset 的活动 Block 标题都必须使用自身强调色的克制渐变，并与活动边框形成同一层级；Workspace 属于应用级导航，当前项使用 accent surface/轮廓/图标与 `text-strong` 文字，不得把 Cyberpunk 的黄色内容 selection marker 混入青色 Workspace 容器。Cyberpunk 的 Settings、连接目标、凭证、文件、页签与 segmented control 内容选中态继续使用黄色轻量 surface 和亮黄标记，未选中功能图标、连接、启用、焦点、浮层边界和滚动反馈使用青色；Files 是局部强调例外，文件行 hover/focus、对应图标、CodeMirror 当前行/行号与编辑光标使用活动 Block 边框同源的低强度亮黄 active surface/marker，但键盘 focus outline 仍为青色。文件列表 selected 行使用同源亮黄一像素内框、轻量黄色 surface 与活动 host 同源红色文本，文件图标和选择标记仍为亮黄；CodeMirror 文本选区使用独立的中强度透明黄 surface 与柔和高明度赛博红文字，保留黄底红字但不得让两个高饱和色处于相近亮度，合成后的文字对比度至少为 4.5:1。CodeMirror 存在非空选区时必须通过 state-derived editor attribute 暂停内容区 current-line surface，保留行号和光标定位反馈，避免半透明选区与当前行底叠加产生局部色差。chrome、utility rail 与 workspace 画布不得用无状态意义的黄色渐变。warning 使用琥珀；除上述 Files selected 文本和活动 host 的主题化强调外，danger 只用于失败、删除、不可逆确认及关闭控件的 hover/pressed，不用于普通选中或装饰。跨主题的文字强调/禁用层级、图标、control hover、Block 边界/header、scrollbar，以及 manager sidebar/editor、card、popover/menu 的 surface 与交互状态，必须由 feature CSS 直接消费语义角色，不得由某一主题的 selector 修补；Light 与 Cyberpunk 常规小字角色在基础 surface 上至少保持 4.5:1。遗留 feature 原始暗色值由仅在 Light root 生效的 `lightOverrides.css` 兼容层隔离，新增 feature CSS 受 raw-color budget 测试约束，后续应继续按功能语义收敛而不是扩大兼容层。
- 文件、编辑器、终端、网络与连接 context menu 共享 `menu text / secondary / disabled / hover` 主题角色。禁用菜单项必须用校准后的实色文字并覆盖全局 button opacity，背景保持透明，图标和快捷键继承同一禁用色；任何 hover/focus 高亮都只能命中 enabled item。Light 兼容层不得重新接管菜单按钮颜色。
- `src/components/Button.tsx` 与 `button.css` 共同拥有文本操作按钮、图标按钮和非交互状态标签的共享视觉与可访问契约。Button variant 表达动作语义、size 表达密度；feature 只追加布局类并继续拥有业务事件。tabs、menu item、选择卡片、列表行和 disclosure control 不得为复用外观而包装成 Button。旧按钮类只允许作为迁移兼容别名，不得再建立独立色板或 Light 主题修补。
- xterm 通过 `src/terminal/terminalTheme.ts` 从 CSS semantic tokens 生成 palette，并以 registry 刷新全部存活实例；CodeMirror 由 Files feature CSS 消费 editor tokens。原生窗口主题只能通过 `src/lib/tauri/window.ts` 的窄 adapter 协调，非关键 native 同步失败不得阻塞 WebView 主题或启动。
...
- settings domain/application 拥有两项可选自动锁定时长、Qterm 配置根规则、封闭的设备外观偏好与启动更新检测偏好。组合根根据 Rust `debug_assertions` 在启动前选择互不 fallback 的 locator/default-root：Development 为 `~/.qterm-location.dev.json` 与 `~/.qterm-dev`，Production 为 `~/.qterm-location.json` 与 `~/.qterm`；随后从所选根派生 `data/`、`device/` 与 `cache/` 并把具体文件路径注入全部 repositories。build mode 与 home 下物理命名不进入 domain、command 或 React；settings snapshot 只投影当前模式的非敏感默认根供恢复默认。外观使用独立 `device/appearance.json` schema v1，只保存 `dark | light | cyberpunk`；更新偏好使用独立 `device/updates.json` schema v1，只保存是否启动检测；二者都不与 `device/settings.json` security schema 共用 record，缺失或读取失败使用安全默认，损坏/未来文件不得覆盖。系统设置 command 只提供窄更新 API；配置根保存只初始化目标分区并更新当前模式 locator，运行时仓库在重启前不切换。更新网络请求、SemVer 比较和短暂提示由 frontend update adapter 与 Workspace shell 编排，不进入 settings domain。credential lifecycle application/runtime coordinator 拥有最近解锁时间、凭证固定有效期 deadline 与统一 lock；React timer、window blur 和 command handler 不拥有凭证有效期规则。
