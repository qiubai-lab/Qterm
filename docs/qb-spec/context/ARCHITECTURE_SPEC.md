...
- settings domain/application 拥有两项可选自动锁定时长与 Qterm 配置根规则。组合根先从用户 home 下固定的 `.qterm-location.json` 读取配置根（默认 `~/.qterm`），再从该根派生 `data/`、`device/` 与 `cache/` 并把具体文件路径注入全部 repositories。系统设置 command 只允许更新整个配置根，并投影配置值、当前启动值和三个只读派生路径；保存只初始化目标分区并更新 locator，运行时仓库在重启前不切换。credential lifecycle application/runtime coordinator 拥有最近解锁时间、凭证固定有效期 deadline 与统一 lock；React timer、window blur 和 command handler 不拥有凭证有效期规则。
