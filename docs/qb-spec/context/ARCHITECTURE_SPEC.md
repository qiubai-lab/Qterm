...
- 私钥正文只存在于 Rust Core 的导入、生成、认证与公钥派生路径，不得进入前端、日志或应用配置文件。生成只开放 Ed25519 与 ECDSA P-256，并在 Rust 内序列化后直接交给既有加密 vault；公钥派生复用 SSH infrastructure 的私钥解析器，窄化 credential IPC 只接收凭证 ID 并只返回标准 OpenSSH 公钥文本。
