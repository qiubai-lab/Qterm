...
- settings domain/application 拥有两项可选自动锁定时长、Qterm 配置根规则、封闭的设备外观偏好与启动更新检测偏好。组合根先从用户 home 下固定的 `.qterm-location.json` 读取配置根（默认 `~/.qterm`），再从该根派生 `data/`、`device/` 与 `cache/` 并把具体文件路径注入全部 repositories。外观使用独立 `device/appearance.json` schema v1，只保存 `dark | light | cyberpunk`；更新偏好使用独立 `device/updates.json` schema v1，只保存是否启动检测；二者都不与 `device/settings.json` security schema 共用 record，缺失或读取失败使用安全默认，损坏/未来文件不得覆盖。系统设置 command 只提供窄更新 API；配置根保存只初始化目标分区并更新 locator，运行时仓库在重启前不切换。更新网络请求、SemVer 比较和短暂提示由 frontend update adapter 与 Workspace shell 编排，不进入 settings domain。credential lifecycle application/runtime coordinator 拥有最近解锁时间、凭证固定有效期 deadline 与统一 lock；React timer、window blur 和 command handler 不拥有凭证有效期规则。
