...
- `document.documentElement[data-theme]` 是唯一 theme DOM owner，`src/app/theme/AppThemeProvider.tsx` 是运行期唯一状态 owner，只允许内置 `dark | light`。Dark 是同步启动 fallback 与缺省值；React 首次渲染前从 Settings snapshot 引导已保存选择，不支持 system/auto/custom theme。
- `src/app/styles/themes/{dark,light}.css` 实现共享 surface/text/border/status、terminal ANSI、editor 与 workbench component semantic token contract。跨主题的文字强调/禁用层级、图标、control hover、Block 边界/header、scrollbar，以及 manager sidebar/editor、card、popover/menu 的 surface 与交互状态，必须由 feature CSS 直接消费语义角色，不得由某一主题的 selector 修补；Light 常规小字角色在基础 surface 上至少保持 4.5:1。遗留 feature 原始暗色值由仅在 Light root 生效的 `lightOverrides.css` 兼容层隔离，新增 feature CSS 受 raw-color budget 测试约束，后续应继续按功能语义收敛而不是扩大兼容层。
...
- settings domain/application 拥有两项可选自动锁定时长、Qterm 配置根规则与封闭的设备外观偏好。组合根先从用户 home 下固定的 `.qterm-location.json` 读取配置根（默认 `~/.qterm`），再从该根派生 `data/`、`device/` 与 `cache/` 并把具体文件路径注入全部 repositories。外观使用独立 `device/appearance.json` schema v1，只保存 `dark | light`，不与 `device/settings.json` security schema 共用 record；缺失或读取失败安全回退 Dark，损坏/未来文件不得覆盖。系统设置 command 只提供窄更新 API；配置根保存只初始化目标分区并更新 locator，运行时仓库在重启前不切换。credential lifecycle application/runtime coordinator 拥有最近解锁时间、凭证固定有效期 deadline 与统一 lock；React timer、window blur 和 command handler 不拥有凭证有效期规则。
