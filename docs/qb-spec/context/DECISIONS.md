...
Qterm 只提供 `dark`、`light` 与 `cyberpunk` 三个封闭预设，Dark 保持缺省且不跟随系统。`AppThemeProvider` 是运行期唯一主题状态 owner，负责设置页 preview/commit/restore，并同步根 DOM、缓存 xterm 和原生窗口；Workspace model 不保存主题。持久化使用当前配置根下独立的 `device/appearance.json` schema v1，不与 security `settings.json` 共用 record，缺失或失败回退 Dark，损坏与未来文件保留不覆盖。三个预设共享 semantic token contract；活动 Block 标题使用各主题强调色的克制渐变。Cyberpunk 以亮黄 `signature/primary-action/selection-marker/selection-surface` 承担品牌、主要操作、活动边框和内容选中，以青色 `accent/focus` 承担顶部当前 Workspace 容器/轮廓/图标、未选中功能图标、连接、启用、浮层边界和滚动反馈；Workspace active text 始终使用高对比 `text-strong`，不混用亮黄。Files 是局部 active 例外：文件行 hover/focus、对应图标、CodeMirror 当前行/行号和编辑光标使用活动 Block 同源的低强度亮黄 surface/marker，键盘 focus outline 仍为青色；文件列表 selected 行采用亮黄一像素内框与轻量黄色 surface，文本采用活动 host 同源红色，图标和选择标记保留亮黄；CodeMirror 文本选区采用中强度透明黄与独立的柔和高明度赛博红，以降低大面积黄色的视觉能量并让小面积文字达到至少 4.5:1 对比度；存在非空文本选区时，选区背景优先于 current-line surface，后者暂停显示但保留行号和光标定位反馈，以避免透明层叠色差。无状态 chrome、utility rail 与 workspace 画布不使用黄色渐变；琥珀承担警告，除活动 host 与 Files selected 文本两个明确例外外，红色只用于失败、删除、不可逆确认及关闭控件的 hover/pressed，native window 映射为 Dark。文字层级、图标、Block chrome、完整 xterm ANSI 与 scrollbar 等跨主题组件角色由 feature CSS 直接消费，常规 Light/Cyberpunk 小字角色在基础 surface 上至少保持 4.5:1；遗留原始颜色仅允许由 Light root-scoped 兼容层承接，并以 raw-color budget 阻止新增债务。

所有文件、编辑器、终端、网络和连接右键菜单统一消费 menu text、secondary、disabled 与 hover 主题角色。禁用项不使用全局半透明叠加，而使用在 raised surface 上至少 4.5:1 的实色文字；其图标和快捷键继承同一层级，并且不响应 hover/focus surface。Light 兼容层不得覆盖这套菜单状态契约。
