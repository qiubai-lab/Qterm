...
- RSA 私钥解析与认证已通过明确风险例外启用。`russh 0.62.6` 固定使用的 `rsa 0.10.0-rc.18` 仍受 RUSTSEC-2023-0071 时序侧信道公告影响；远程观察者可能通过大量签名时序样本尝试恢复私钥。凭证 UI 必须标记 RSA 为“不安全”，认证只使用 RSA-SHA2-512/256，禁止 SHA-1 `ssh-rsa` 降级。
...
- RSA 是用户明确接受的已知安全例外：当前纯 Rust RSA 签名依赖没有 RUSTSEC-2023-0071 的修复版本。应优先迁移到 Ed25519/ECDSA，并在上游修复后重新审计和升级。
